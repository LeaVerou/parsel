...
const TOKENS = {
    [TokenType.Attribute]: /\[\s*(?:(?<namespace>\*|[-\w]*)\|)?(?<name>[-\w\u{0080}-\u{FFFF}]+)\s*(?:(?<operator>\W?=)\s*(?<value>.+?)\s*(\s(?<caseSensitive>[iIsS]))?\s*)?\]/gu,
    ...
    [TokenType.PseudoElement]: /::(?<name>[-\w\u{0080}-\u{FFFF}]+)(?:\((?<argument>¶+)\))?/gu,
    [TokenType.PseudoClass]: /:(?<name>[-\w\u{0080}-\u{FFFF}]+)(?:\((?<argument>¶+)\))?/gu,
    [TokenType.Universal]: /(?:(?<namespace>\*|[-\w]*)\|)?\*/gu,
    [TokenType.Type]: /(?:(?<namespace>\*|[-\w]*)\|)?(?<name>[-\w\u{0080}-\u{FFFF}]+)|\*/gu // this must be last
};
...
export { RECURSIVE_PSEUDO_CLASSES, RECURSIVE_PSEUDO_CLASSES_ARGS, TOKENS, TOKENS_TO_TRIM, TOKENS_WITH_PARENS, TOKENS_WITH_STRINGS, TokenType, parse, specificity, specificityToNumber, tokenize, tokenizeBy, walk };
